...
# CoSolvKit
#
# Structural QC: RMSF/RMSD, equilibration statistics and RDFs.
#
...
def _plot_rmsf(rmsf_df, out_path):
    """Plot per-residue RMSF (Å, mean over the residue's atoms) to ``rmsf_by_residue.png``.
...
def _plot_rmsd(rmsd_df, avg_selection, out_path):
    """Plot per-frame RMSD (Å) to the average structure, as ``rmsd_by_frame.png``.
...
def rmsf_analysis(universe, out_path, avg_selection, align_selection):
    """Compute per-atom RMSF (Å) and the trajectory average structure.

    Assumes the trajectory is already aligned.  Writes ``rmsf_by_atom.csv``,
    ``rmsd_by_frame.csv``, their plots, and ``averaged_trajectory.pdb`` with
    RMSF stored in the B-factor column — that PDB is the reference structure
    ...
    # from here on avg_selection is heavy-atom only; RMSF is never over hydrogens
    avg_selection = avg_selection + ' and not name H*'
...
def equilibration_analysis(statistics_file, out_path):
    """Plot potential energy, volume and temperature to ``equilibration_statistics.png``.

    :param statistics_file: log file generated by the MD simulation, i.e. statistics.csv.
...
def _get_temp_vol_pot(statistics_file):
    """Read per-frame potential energy (kJ/mol), temperature (K) and box volume (nm^3).

    :param statistics_file: log file generated by the MD simulation.
    :type statistics_file: str
...
def rdf_analysis(universe, cosolvent_names, out_path):
    """Plot RDFs and their autocorrelations into an ``RDFs`` subdirectory.

    One ``rdf_{cosolvent}_{atom}.png`` per heavy atom type (cosolvent-cosolvent
    and cosolvent-water panels), plus ``rdf_HOH_O.png``.  Waters are selected by
    the resname ``HOH``.
    ...
    for cosolvent_name in cosolvent_names:
        ...
        for cosolvent_atom in set(atoms_names):
            ...
            # same atom type in different molecules (exclusion_block drops self-pairs)
            ...
            # cosolvent atom vs water oxygen
...
def autocorrelation(data):
    """Autocorrelation of *data* at non-negative lags (unnormalised).
...
def _plot_autocorrelation(data, ax, cosolvent_name1=None, cosolvent_atom1=None, cosolvent_name2=None, cosolvent_atom2=None):
    """Draw the autocorrelation of *data* onto *ax*, normalised to a max of 1.
    ...
    :param cosolvent_name1: molecule/atom names of the two RDF partners, used for the
        title only, defaults to None
