...
def test_kt_mode_default_is_minus_one_kt():
    cfg = HotspotsConfig()
    assert cfg.cutoff_mode == "kt"
    assert cfg.n_kt == 1.0
    got = resolve_agfe_cutoff(cfg, temperature=300.0)
    assert got == pytest.approx(-1.0 * BOLTZMANN_CONSTANT_KB * 300.0, abs=1e-9)  # ~ -0.5962


def test_kt_mode_scales_with_temperature_and_nkt():
    cfg = HotspotsConfig(cutoff_mode="kt", n_kt=2.0)
    assert resolve_agfe_cutoff(cfg, 310.0) == pytest.approx(-2.0 * BOLTZMANN_CONSTANT_KB * 310.0, abs=1e-9)
