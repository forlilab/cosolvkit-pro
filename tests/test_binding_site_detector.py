...
def _hs(cosolvent, site_id, blob, agfe_min, atoms, sp_half_life=None, shape=(20, 20, 20)):
    ...
    if sp_half_life is not None:
        # KINETICS_METRIC: residence reads sp_half_life (fraction still bound at long
        # lag), not sp_mrt, which is censored at tau_max for the strongest sites.
        h.add_property("sp_half_life", sp_half_life)
    return h


def test_build_binding_site_aggregates_members():
    a = _hs("BEN", 1, np.s_[5:9, 5:9, 5:9], -3.0, ["Car"], sp_half_life=0.80)
    b = _hs("IMI", 2, np.s_[7:11, 7:11, 7:11], -2.0, ["HBA"], sp_half_life=0.40)
    ...
    assert bs.residence == 0.80                      # max sp_half_life across members
    assert bs.residence_metric == "sp_half_life"
...
def test_identify_binding_sites_ranks_and_ids():
    ...
    a1 = _hs("BEN", 1, np.s_[5:9, 5:9, 5:9], -3.0, ["Car"], sp_half_life=0.80)
    a2 = _hs("IMI", 2, np.s_[7:11, 7:11, 7:11], -3.0, ["HBA"], sp_half_life=0.60)
    b1 = _hs("BEN", 3, np.s_[15:18, 15:18, 15:18], -1.0, ["Car"], sp_half_life=0.10)
