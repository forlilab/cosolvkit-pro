...
def test_identity_resample_when_same_grid():
    mask = np.zeros((6, 6, 6), dtype=bool)
    mask[2:4, 2:4, 2:4] = True
    origin = np.array([0.0, 0.0, 0.0]); delta = np.array([0.5, 0.5, 0.5])
    out = resample_mask_to_grid(mask, origin, delta, origin, delta, (6, 6, 6))
